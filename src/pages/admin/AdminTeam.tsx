              ...
              <>
                ...
                {editing.image_url ? (
                  <div className="overflow-hidden rounded-[1.5rem] border border-slate-200 bg-slate-50 p-3">
                    <img src={buildRawImageUrl(editing.image_url, { width: 480, quality: 84 })} alt={editing.name || "Team Preview"} className="h-48 w-full rounded-[1rem] object-cover" />
                  </div>
                ) : null}
          ...
          <Card key={member.id} className="rounded-[1.75rem] border-slate-200">
            <CardContent className="flex gap-4 p-5">
              <div className="h-24 w-24 shrink-0 overflow-hidden rounded-[1.25rem] bg-slate-100">
                {member.image_url ? (
                  <img src={buildRawImageUrl(member.image_url, { width: 240, quality: 82 })} alt={member.name} className="h-full w-full object-cover" />
                ) : (
