...
import { createDefaultSupportSettings, SUPPORT_MODE_OPTIONS, type SiteSupportSettingsRecord } from '@/lib/support';
...
const AdminSites = () => {
  ...
  const { organizations, createOrganization } = useSupportOrganizations();
  const [form, setForm] = useState<SiteForm>(emptyForm);
  const [orgForm, setOrgForm] = useState({ name: '', type: 'agency', siteId: '' });
  ...
  const { data: supportSettingsRows = [], isLoading: supportSettingsLoading } = useQuery({
    queryKey: ['site-support-settings-admin', isGlobalAdmin],
    enabled: isGlobalAdmin,
    queryFn: async () => {
      const { data, error } = await supabase.from('site_support_settings' as never).select('*');
      if (error) {
        const code = typeof (error as any)?.code === 'string' ? (error as any).code : '';
  ...
  const supportSettingsBySiteId = useMemo(
    () => new Map(supportSettingsRows.map((row) => [String(row.site_id), row])),
    [supportSettingsRows],
  );
  ...
  const saveSupportSettings = useMutation({
    mutationFn: async ({ siteId, patch }: { siteId: string; patch: Partial<SiteSupportSettingsRecord> }) => {
      const current = supportSettingsBySiteId.get(siteId) ?? createDefaultSupportSettings(siteId);
      const payload = {
        site_id: siteId,
        support_mode: patch.support_mode ?? current.support_mode,
        support_organization_id: patch.support_organization_id ?? current.support_organization_id ?? null,
        allow_platform_escalation: typeof patch.allow_platform_escalation === 'boolean' ? patch.allow_platform_escalation : Boolean((current as any).allow_platform_escalation),
        support_widget_enabled: typeof patch.support_widget_enabled === 'boolean' ? patch.support_widget_enabled : current.support_widget_enabled,
        support_email_enabled: typeof patch.support_email_enabled === 'boolean' ? patch.support_email_enabled : current.support_email_enabled,
        default_sla_hours: typeof patch.default_sla_hours === 'number' ? patch.default_sla_hours : Number((current as any).default_sla_hours ?? 24),
      };
      const { error } = await supabase.from('site_support_settings' as never).upsert(payload as never, { onConflict: 'site_id' });
      if (error) throw error;
    },
    onSuccess: async () => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: ['site-support-settings-admin'] }),
        qc.invalidateQueries({ queryKey: ['site-support-settings'] }),
        qc.invalidateQueries({ queryKey: ['public-support-settings'] }),
      ]);
      toast.success('Support-Routing gespeichert.');
    },
    onError: (error: any) => toast.error(error?.message || 'Support-Routing konnte nicht gespeichert werden.'),
  });
  ...
  const saveSite = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        ...
        qc.invalidateQueries({ queryKey: ['site-modules-admin'] }),
        qc.invalidateQueries({ queryKey: ['site-support-settings-admin'] }),
        qc.invalidateQueries({ queryKey: ['support-organizations'] }),
      ]);
      refetchSites();
      setForm(emptyForm);
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          ...
          {isGlobalAdmin ? (
            <Button variant="ghost" size="sm" className="rounded-xl" onClick={() => setForm({ id: row.original.id, name: row.original.name || '', slug: row.original.slug || '', primary_domain: row.original.primary_domain || row.original.site_domains?.find((entry) => entry.is_primary)?.hostname || '', description: row.original.description || '', is_default: !!row.original.is_default, is_active: !!row.original.is_active })}>
              Bearbeiten
            </Button>
          ) : null}
        </div>
    ...
    <div className="max-w-7xl p-6 md:p-8 lg:p-10">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">Mandanten, Domains, Module und Support-Routing auf einer zentralen SaaS-Schiene.</p>
        </div>
      ...
      {isGlobalAdmin ? (
        <section className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-6 flex items-center gap-3 text-lg font-bold text-slate-900">
              <LifeBuoy size={20} className="text-[#FF4B2C]" /> Support-Organisationen
            </div>
            <div className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2 md:col-span-2"><Label>Name</Label><Input value={orgForm.name} onChange={(event) => setOrgForm((prev) => ({ ...prev, name: event.target.value }))} className="rounded-xl border-slate-200" placeholder="z. B. Agentur Huber Support" /></div>
                <div className="space-y-2"><Label>Typ</Label>
                  <Select value={orgForm.type} onValueChange={(value) => setOrgForm((prev) => ({ ...prev, type: value }))}>
                ...
                <div className="space-y-2"><Label>Site-Verknüpfung optional</Label>
                  <Select value={orgForm.siteId || 'none'} onValueChange={(value) => setOrgForm((prev) => ({ ...prev, siteId: value === 'none' ? '' : value }))}>
                    <SelectTrigger className="rounded-xl border-slate-200"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="none">Keine feste Site</SelectItem>
              ...
              <Button
                ...
                disabled={createOrganization.isPending || !orgForm.name.trim()}
                onClick={() => createOrganization.mutate({ name: orgForm.name.trim(), type: orgForm.type as any, siteId: orgForm.siteId || null }, {
                  onSuccess: () => {
                    setOrgForm({ name: '', type: 'agency', siteId: '' });
                    toast.success('Support-Organisation erstellt.');
                  },
                  onError: (error: any) => toast.error(error?.message || 'Organisation konnte nicht erstellt werden.'),
                })}
              >Support-Organisation anlegen</Button>
            </div>

            <div className="mt-6 space-y-3">
              {organizations.length ? organizations.map((entry) => (
                <div key={entry.id} className="rounded-[1.25rem] border border-slate-200 bg-slate-50 px-4 py-4">
                  <div className="text-sm font-bold text-slate-900">{entry.name}</div>
                  <div className="mt-1 text-xs uppercase tracking-[0.18em] text-slate-400">{entry.type} · {entry.slug}</div>
                </div>
              )) : <div className="rounded-[1.25rem] border border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">Noch keine Support-Organisationen.</div>}
            </div>
          </div>

          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="mb-6 text-lg font-bold text-slate-900">Support-Routing pro Site</div>
            {supportSettingsLoading ? (
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-6 py-8 text-sm text-slate-500">Support-Settings werden geladen…</div>
            ) : (
              <div className="space-y-4">
                {sites.map((site) => {
                  const settings = { ...createDefaultSupportSettings(site.id), ...(supportSettingsBySiteId.get(site.id) || {}) } as SiteSupportSettingsRecord & Record<string, unknown>;
                  const relevantOrganizations = organizations.filter((entry) => entry.type === 'platform' || entry.site_id === site.id || entry.type === 'agency');
                  return (
                    <article key={`support-${site.id}`} className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5">
                      ...
                      <div className="mt-5 grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                          <Label>Support-Modus</Label>
                          <Select value={settings.support_mode} onValueChange={(value) => saveSupportSettings.mutate({ siteId: site.id, patch: { support_mode: value as any } })}>
                            <SelectTrigger className="rounded-xl border-slate-200 bg-white"><SelectValue /></SelectTrigger>
                            <SelectContent>{SUPPORT_MODE_OPTIONS.map((entry) => <SelectItem key={entry.value} value={entry.value}>{entry.label}</SelectItem>)}</SelectContent>
                          </Select>
                        </div>
                        <div className="space-y-2">
                          <Label>Ziel-Organisation</Label>
                          <Select value={settings.support_organization_id || 'none'} onValueChange={(value) => saveSupportSettings.mutate({ siteId: site.id, patch: { support_organization_id: value === 'none' ? null : value } })}>
                            <SelectTrigger className="rounded-xl border-slate-200 bg-white"><SelectValue placeholder="Organisation wählen" /></SelectTrigger>
                            <SelectContent>
                              <SelectItem value="none">Automatisch / Fallback</SelectItem>
                              {relevantOrganizations.map((entry) => <SelectItem key={entry.id} value={entry.id}>{entry.name}</SelectItem>)}
                            </SelectContent>
                          </Select>
                        </div>
                        <div className="space-y-2">
                          <Label>SLA in Stunden</Label>
                          <Input type="number" min={1} max={720} value={String(settings.default_sla_hours ?? 24)} onChange={(event) => saveSupportSettings.mutate({ siteId: site.id, patch: { default_sla_hours: Number(event.target.value || 24) } })} className="rounded-xl border-slate-200 bg-white" />
                        </div>
                      </div>

                      <div className="mt-5 flex flex-wrap gap-4">
                        <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700">
                          <Switch checked={Boolean(settings.support_widget_enabled)} onCheckedChange={(checked) => saveSupportSettings.mutate({ siteId: site.id, patch: { support_widget_enabled: checked } })} />
                          Public Widget aktiv
                        </label>
                        <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700">
                          <Switch checked={Boolean(settings.support_email_enabled)} onCheckedChange={(checked) => saveSupportSettings.mutate({ siteId: site.id, patch: { support_email_enabled: checked } })} />
                          E-Mail-Kanal aktiv
                        </label>
                        <label className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700">
                          <Switch checked={Boolean(settings.allow_platform_escalation)} onCheckedChange={(checked) => saveSupportSettings.mutate({ siteId: site.id, patch: { allow_platform_escalation: checked } })} />
                          Plattform-Eskalation aktiv
                        </label>
                      </div>
                    </article>
                  );
