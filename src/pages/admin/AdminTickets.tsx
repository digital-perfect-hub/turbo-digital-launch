...
const AdminTickets = () => {
  ...
  // FIX 1: Kein Auto-Select mehr. Nur noch Deselect, wenn das Ticket z.B. gelöscht oder weggefiltert wird.
  useEffect(() => {
  ...
  // Scroll nach unten beim Öffnen
  const handleSelectTicket = (id: string) => {
    setSelectedTicketId(id);
    setTimeout(() => {
      document.getElementById('ticket-detail-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 100);
  };

  // FIX 3: Scroll nach oben beim Schließen
  const handleCloseTicket = () => {
    setSelectedTicketId(null);
    setTimeout(() => {
      document.getElementById('ticket-list-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 50);
  };
  ...
  return (
    <div className="mx-auto w-full min-w-0 max-w-[1600px] p-6 md:p-8 lg:p-10">
      <div className="mb-8 flex w-full min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0 flex-1">
          ...
          <h1 className="mt-4 truncate text-3xl font-extrabold tracking-[-0.04em] text-slate-900">Tickets & Eskalation</h1>
          <p className="mt-2 w-full min-w-0 truncate text-sm text-slate-500">Mandantenfähiger Support auf Basis eurer Site- und Rollenlogik. Aktueller Modus: <strong>{settings.support_mode}</strong>.</p>
        </div>

        <Dialog open={createOpen} onOpenChange={setCreateOpen}>
          <DialogTrigger asChild>
            <Button className="shrink-0 rounded-xl bg-[#FF4B2C] text-white hover:bg-[#E03A1E]"><Plus size={16} className="mr-2" /> Ticket anlegen</Button>
          </DialogTrigger>
      ...
      <Card className="mt-6 w-full min-w-0 rounded-[2rem] border border-slate-200 bg-white shadow-sm">
      ...
      <div className="mt-8 flex w-full min-w-0 flex-col gap-12">
        {/* Ticket-Liste */}
        <section id="ticket-list-section" className="flex w-full min-w-0 flex-col scroll-mt-8">
          {isLoading ? (
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-sm">Tickets werden geladen…</div>
          ) : (
            <TicketListTable tickets={filteredTickets} selectedTicketId={selectedTicketId} onSelect={handleSelectTicket} />
          )}
        </section>

        {/* FIX 2: Conditional Rendering - Panel lädt nur, wenn ein Ticket ausgewählt ist */}
        {selectedTicketId && (
          <>
            <div className="h-px w-full bg-slate-200" /> {/* Optische Trennlinie */}

            <section id="ticket-detail-section" className="flex w-full min-w-0 flex-col scroll-mt-8">
              {/* FIX 3: Close Button */}
              <div className="mb-4 flex items-center justify-end">
                <Button 
                  variant="outline" 
                  className="rounded-xl border-slate-200 bg-white text-slate-600 shadow-sm hover:bg-slate-50 hover:text-slate-900" 
                  onClick={handleCloseTicket}
                >
                  <X size={16} className="mr-2" /> Ansicht schließen
                </Button>
              </div>
