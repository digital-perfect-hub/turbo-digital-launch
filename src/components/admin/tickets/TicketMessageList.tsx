    ...
    <div className="flex w-full min-w-0 flex-col space-y-3">
      {messages.map((message) => (
        <article
          key={message.id}
          className={cn(
            'flex w-full min-w-0 flex-col overflow-hidden rounded-[1.25rem] border px-4 py-4',
            message.is_internal_note ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white',
          )}
        >
          <div className="flex w-full min-w-0 flex-wrap items-center gap-3 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">
            <span className="max-w-[200px] truncate sm:max-w-[300px]">{message.author_type}</span>
            ...
            <span className="shrink-0">{new Date(message.created_at || Date.now()).toLocaleString('de-AT')}</span>
          </div>
          <div className="mt-3 w-full min-w-0 whitespace-pre-wrap break-words text-sm leading-7 text-slate-700 [word-break:break-word] [overflow-wrap:anywhere]">
            {message.message}
          </div>
        </article>
      ))}
