  ...
  return (
    <div className="overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-sm">
      ...
      <div className="divide-y divide-slate-100">
        {tickets.map((ticket) => (
          <div key={ticket.id} className={cn('grid gap-4 px-5 py-4 lg:grid-cols-[minmax(0,1.4fr)_0.7fr_0.7fr_0.7fr_auto] lg:items-center', selectedTicketId === ticket.id && 'bg-[#FFF9F7]')}>
            <div className="min-w-0">
              <div className="truncate text-base font-bold text-slate-900">{ticket.subject}</div>
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-sm text-slate-500">
                <span>{ticket.requester_name}</span>
                <span>{ticket.requester_email}</span>
                <span>{new Date(ticket.created_at || Date.now()).toLocaleDateString('de-AT')}</span>
              </div>
            </div>
            <TicketStatusBadge value={ticket.status} />
            <TicketPriorityBadge value={ticket.priority} />
            <div className="text-sm font-medium text-slate-600">{getTicketCategoryLabel(ticket.category)}</div>
            <div>
              <Button variant={selectedTicketId === ticket.id ? 'default' : 'outline'} className={cn('rounded-xl', selectedTicketId === ticket.id ? 'bg-[#FF4B2C] text-white hover:bg-[#E03A1E]' : 'border-slate-200')} onClick={() => onSelect(ticket.id)}>
                Öffnen
              </Button>
            </div>
          </div>
